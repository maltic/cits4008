...
much worse asymptotic and implementation complexity. It was later improved by Matthews et al. \cite{mathews1999expanded, mathews2004incorporating}. The time complexity of the Zuker algorithm is $O(n^3)$. The value $n$ refers to the number of nucleotides in a RNA molecule. RNAs are essentially chains of nucleotides, thus it is natural that any prediction algorithm should scale as the number of nucleotides increases. In short, the Zuker algorithm is able to efficiently predict structures for large RNAs on modern hardware.  Because of
